const {
	generateSalt,
	hmac
} = require('../tools/crypto-hash-tool.js')
...
class Frontend {
	constructor() {
		...
		this.result = { /* account:null, password: null, newPassword: null, newSalt:null */ } // Result after slow-hash
	}
	...
	slowHash() {
		...
		// 	password: hmac(this.data.salt, this.data.password, {
		// 		alg: 'sha256',
		// 		repeat: 100
		// 	}),
		// 	newSalt: generateSalt(16),
		// 	newPassword: hmac(this.result.newSalt, this.data.password, {
		// 		alg: 'sha256',
		// 		repeat: 100
		// 	})
		// }
		...
		this.result.password = hmac(this.data.salt, this.data.password, {
		alg: 'sha256',
		repeat: 100
		}),
		this.result.newSalt = generateSalt(16),
		this.result.newPassword = hmac(this.result.newSalt, this.data.password, {
			alg: 'sha256',
			repeat: 100
		})
	}
